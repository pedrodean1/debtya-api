...
app.post("/plaid/create_link_token", async (req, res) => {
  try {
    ...
    if (!user_id) return res.status(400).json({ error: true, message: "user_id is required" });
    ...
    return res.status(err?.response?.status || 500).json(plaidData || { error: true, message: err.message });
  }
});
...
app.post("/plaid/exchange_public_token", async (req, res) => {
  try {
    ...
    if (!user_id || !public_token) {
      return res.status(400).json({ error: true, message: "user_id and public_token are required" });
    }
