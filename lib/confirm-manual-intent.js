...
function createConfirmManualPaymentIntentHandler(deps) {
  ...
  async function returnExecutedIntentIdempotentOrRecover(
    userId,
    intentId,
    cur,
    amount,
    ...
    if (!appliedOk) {
      ...
      const err = new Error(`No se pudo rebajar el balance de la deuda tras confirmar el pago (${detail}).`);
      err.status = 409;
  ...
  return async function confirmManualPaymentIntentDirect(userId, intentId, options = {}) {
    ...
    if (st0 === "executed") {
      return returnExecutedIntentIdempotentOrRecover(
        userId,
        intentId,
        intent,
        amount,
