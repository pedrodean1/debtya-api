 ...
 * Sin tocar RPC build_intents_v2 — reconcileManualFirstPriorityIntent en server.js cancela intents
 * abiertos competidores e inserta un único pending_review con metadata.manual_first_priority.
 */
...
function debtBalanceForPlan(debt, safeNumber) {
  return safeNumber(debt?.balance ?? debt?.current_balance);
}
...
function pickPriorityDebtForManualPlan(strategy, debts, safeNumber) {
  const active = (debts || []).filter((d) => debtRowEligibleForPlan(d, safeNumber));
  ...
  if (s === "snowball") {
    active.sort((a, b) => {
      const db = debtBalanceForPlan(a, safeNumber) - debtBalanceForPlan(b, safeNumber);
      if (db !== 0) return db;
      const aprDiff =
  ...
  if (anyPositiveApr) {
    active.sort((a, b) => {
      ...
      return debtBalanceForPlan(b, safeNumber) - debtBalanceForPlan(a, safeNumber);
    });
  } else {
    active.sort((a, b) => {
      const bal = debtBalanceForPlan(b, safeNumber) - debtBalanceForPlan(a, safeNumber);
      if (bal !== 0) return bal;
...
function computeManualPriorityPaymentAmount(priorityDebt, plan, safeNumber) {
  const bal = debtBalanceForPlan(priorityDebt, safeNumber);
  if (!(bal > 0)) return 0;
