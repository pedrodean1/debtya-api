...
async function executeManualPlanRebuild(deps) {
  const { userId, body, supabaseAdmin, getCurrentPaymentPlan, normalizePaymentPlan, safeNumber, isUuid } =
    deps;
  ...
  const debts = (debtRows || []).filter((d) => d && safeNumber(d.balance) > 0);
  ...
  if (toCancel.length) {
    ...
    await supabaseAdmin
      ...
      .update({
        status: "canceled",
        updated_at: now,
        notes: "Cancelado por POST /manual-plan/rebuild (manual-first)"
      })
      .in("id", ids)
      .eq("user_id", userId);
  }
    ...
    if (strayIds.length) {
      await supabaseAdmin
        ...
        .update({
          status: "canceled",
          updated_at: now,
          notes: "Mantenido solo intent manual-first (post-insert /manual-plan/rebuild)"
        })
        .in("id", strayIds)
        .eq("user_id", userId);
