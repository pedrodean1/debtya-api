...
function defaultNotificationPreferences(userId = null) {
  return {
    ...
    auto_track_minimum_payments: false,
...
function normalizeNotificationPreferences(row, userId = null) {
  ...
  return {
    ...
    auto_track_minimum_payments: boolValue(source.auto_track_minimum_payments),
...
function validateNotificationPreferencesInput(body, existing = null, nowIso = new Date().toISOString()) {
  ...
  const autoTrackMinimumPayments =
    "auto_track_minimum_payments" in src
      ? boolValue(src.auto_track_minimum_payments)
      : !!previous.auto_track_minimum_payments;
  ...
  return {
    payload: {
      ...
      auto_track_minimum_payments: autoTrackMinimumPayments,
...
async function findOpenManualFirstIntents(supabaseAdmin, userId) {
  const { data, error } = await supabaseAdmin
  ...
  return rows.filter(isManualFirstIntent);
}

function normalizeReminderPaymentPlan(row, fallbackStrategy = "avalanche") {
  const source = row && typeof row === "object" ? row : {};
  const payload = source.payload_json && typeof source.payload_json === "object" ? source.payload_json : {};
  const strategy = safeString(source.strategy || payload.strategy || fallbackStrategy || "avalanche", 30).toLowerCase();
  const monthlyBudget =
    source.monthly_budget !== undefined && source.monthly_budget !== null
      ? safeNumber(source.monthly_budget, 0)
      : payload.monthly_budget !== undefined
      ? safeNumber(payload.monthly_budget, 0)
      : safeNumber(source.monthly_budget_default ?? payload.monthly_budget_default, 0);
  const monthlyBudgetDefault =
    source.monthly_budget_default !== undefined && source.monthly_budget_default !== null
      ? safeNumber(source.monthly_budget_default, 0)
      : payload.monthly_budget_default !== undefined
      ? safeNumber(payload.monthly_budget_default, 0)
      : monthlyBudget;
  const extraPaymentDefault =
    source.extra_payment_default !== undefined && source.extra_payment_default !== null
      ? safeNumber(source.extra_payment_default, 0)
      : safeNumber(payload.extra_payment_default, 0);
  const targetDebtId = safeString(source.payment_target_debt_id || payload.payment_target_debt_id || "", 80) || null;
  return {
    ...source,
...
async function fetchCurrentPaymentPlanForReminder(supabaseAdmin, userId, fallbackStrategy) {
  const { data, error } = await supabaseAdmin
    .from("payment_plans")
    .select("*")
    .eq("user_id", userId)
    ...
    .maybeSingle();
  if (error) throw error;
  return normalizeReminderPaymentPlan(data, fallbackStrategy);
}

async function fetchCurrentDebtRowsForReminder(supabaseAdmin, userId) {
  const { data, error } = await supabaseAdmin
    .from("debts")
    .select("*")
    .eq("user_id", userId)
...
function debtCurrentBalance(debt) {
  return safeNumber(debt?.balance ?? debt?.current_balance, 0);
}

function findMatchingManualFirstIntentForDebt(intents, debtId) {
  const id = safeString(debtId, 80);
  if (!id) return null;
  return (intents || []).find((intent) => safeString(intent?.debt_id, 80) === id) || null;
}

function countStalePaidReminderIntents(intents, debtRows) {
  const byId = new Map((debtRows || []).map((row) => [safeString(row?.id, 80), row]));
  let count = 0;
  for (const intent of intents || []) {
    const debtId = safeString(intent?.debt_id, 80);
    const debt = byId.get(debtId);
    if (debt && !debtRowEligibleForPlan(debt, safeNumber)) count += 1;
  }
  return count;
}

function selectPriorityDebtForReminder(plan, debtRows) {
  const activeDebts = (debtRows || []).filter((debt) => debtRowEligibleForPlan(debt, safeNumber));
  if (!activeDebts.length) return null;
  const targetId = safeString(plan?.payment_target_debt_id, 80);
  if (targetId) {
    const locked = activeDebts.find((debt) => safeString(debt?.id, 80) === targetId);
    if (locked) return locked;
  }
  return pickPriorityDebtForManualPlan(plan?.strategy || "avalanche", activeDebts, safeNumber);
}

async function resolveCurrentReminderRecommendation(opts) {
  const { supabaseAdmin, userId, getIntentAmount } = opts || {};
  const intents = await findOpenManualFirstIntents(supabaseAdmin, userId);
  const intentStrategy = intents.find((intent) => safeString(intent?.strategy, 30))?.strategy;
  const plan = await fetchCurrentPaymentPlanForReminder(supabaseAdmin, userId, intentStrategy);
  const debtRows = await fetchCurrentDebtRowsForReminder(supabaseAdmin, userId);
  const skippedPaidDebtCount = countStalePaidReminderIntents(intents, debtRows);
  const debt = selectPriorityDebtForReminder(plan, debtRows);
  if (!debt) {
    return {
      ok: true,
      reason: "no_active_debts",
      plan,
      intent: null,
      debt: null,
      amount: null,
      current_balance: null,
      skipped_paid_debt_count: skippedPaidDebtCount
    };
  }

  const matchingIntent = findMatchingManualFirstIntentForDebt(intents, debt.id);
  const currentBalance = debtCurrentBalance(debt);
  const computedAmount = computeManualPriorityPaymentAmount(debt, plan, safeNumber);
  const intentAmount = matchingIntent ? getAmountFromIntent(matchingIntent, getIntentAmount) : 0;
  const amountBase = intentAmount > 0 ? intentAmount : computedAmount;
  const amount = Math.min(currentBalance, amountBase);
  if (!Number.isFinite(amount) || amount <= 0) {
    return {
      ok: false,
      reason: "next_intent_invalid_amount",
      plan,
...
function buildNoActiveDebtsReminderPreview({ channel, preferredLanguage, appUrl = "https://www.debtya.com" }) {
  const langResolved = normalizePreferredLanguage(preferredLanguage || "en");
  const es = langResolved === "es";
  const normalizedChannel = VALID_CHANNELS.has(channel) ? channel : "email";
  const line = es
    ? "Estás al día. No tienes deudas activas para dar seguimiento en este momento."
    : "You’re all caught up. You don’t have any active debts to track right now.";
  const subject = es ? "DebtYa: estás al día" : "DebtYa: you’re all caught up";
  const bodyLines = es
    ? ["Hola,", "", line, "", `Donde hacerlo cuando quieras: ${appUrl}`]
    : ["Hello,", "", line, "", `When you are ready: ${appUrl}`];
  const body = bodyLines.join("\n");
  return {
    channel: normalizedChannel,
    subject,
    message: normalizedChannel === "sms" ? `DebtYa: ${line}` : body,
    sms_message: `DebtYa: ${line}`,
    email_body: body,
    intent_id: null,
    debt_id: null,
    debt_name: null,
...
async function buildNextPaymentReminderPreview(opts) {
  ...
  const recommendation = await resolveCurrentReminderRecommendation({
    supabaseAdmin,
    userId,
    getIntentAmount
  });
  if (recommendation.reason === "no_active_debts") {
    const preview = buildNoActiveDebtsReminderPreview({ channel, preferredLanguage: langResolved, appUrl });
    preview.skipped_paid_debt_count = recommendation.skipped_paid_debt_count || 0;
    return preview;
  }
  if (!recommendation.ok || !Number.isFinite(recommendation.amount) || recommendation.amount <= 0) {
    const err = new Error("Reminder intent has no valid amount");
    err.status = 400;
    throw err;
  }

  const { debt, intent, plan, amount, current_balance: currentBalance } = recommendation;
  ...
  const currentBalanceText = `$${currentBalance.toFixed(2)}`;
  const sms = es
    ...
    : `DebtYa: suggested payment ${amountText} (${debtNameDisplay}, current balance ${currentBalanceText}). Pay your creditor or bank directly, then in DebtYa tap "I paid it".`;
  const subject = es ? "DebtYa: recordatorio de pago sugerido" : "DebtYa: your payment reminder";
  const bodyLinesEs = [
    "Hola,",
    "",
    `Deuda recomendada: ${debtNameDisplay}`,
    `Balance actual: ${currentBalanceText}`,
  ...
  const bodyLinesEn = [
    ...
    `Current balance: ${currentBalanceText}`,
  ...
  return {
    ...
    debt_id: debt?.id || intent?.debt_id || null,
    debt_name: debtNameDisplay,
    amount,
    current_balance: currentBalance,
    strategy: String(plan?.strategy || "avalanche").toLowerCase(),
    skipped_paid_debt_count: recommendation.skipped_paid_debt_count || 0
  };
}
...
function incrementReminderReason(summary, reason) {
  summary.skipped += 1;
  countReminderReason(summary, reason);
}

function countReminderReason(summary, reason) {
...
async function buildReminderDebugState(opts) {
  ...
  try {
    recommendation = await resolveCurrentReminderRecommendation({
      supabaseAdmin,
      userId,
      getIntentAmount
    });
  ...
  const intent = recommendation?.intent || null;
  const amount = recommendation?.amount ?? null;
  ...
  else if (recommendation?.reason === "no_active_debts") reason = "no_active_debts";
  else if (!recommendation) reason = "notification_lookup_failed";
  ...
  return {
    ...
    has_active_debt_recommendation: recommendation?.reason === "selected_next_debt",
    next_intent_status: intent?.status || null,
    next_intent_amount: Number.isFinite(amount) && amount > 0 ? amount : null,
    next_debt_id: recommendation?.debt?.id || null,
    next_debt_current_balance:
      Number.isFinite(recommendation?.current_balance) && recommendation.current_balance > 0
        ? recommendation.current_balance
        : null,
...
async function runDuePaymentReminders(deps) {
  ...
  for (const row of rows || []) {
    ...
    if (!prefs.email_enabled) {
      incrementReminderReason(summary, "email_not_enabled");
      continue;
    }
    if (forceTest) {
    ...
    for (const channel of channels) {
      ...
      try {
        ...
        if (preview.no_active_debts) {
          incrementReminderReason(summary, "no_active_debts");
          if (preview.skipped_paid_debt_count > 0) countReminderReason(summary, "skipped_paid_debt");
          continue;
        }
        countReminderReason(summary, "selected_next_debt");
        if (preview.skipped_paid_debt_count > 0) countReminderReason(summary, "skipped_paid_debt");
        ...
        if (sendResult.sent) {
          await insertAutoReminderEvent(supabaseAdmin, {
            ...
            intent_id: preview.intent_id || null,
            channel,
            preview,
            metadata: forceTest ? { force_test: true } : undefined
          });
          summary.sent += 1;
          countReminderReason(summary, "reminder_sent");
          break;
        }
        incrementReminderReason(summary, "provider_returned_not_sent");
        countReminderReason(summary, "reminder_failed");
      } catch (e) {
        countReminderReason(summary, "reminder_failed");
        summary.errors.push(
          cronErrorSummary({
...
function buildPaymentRecordedTransactionalCopy(lang, amount, debtNameRaw) {
  ...
  if (lang === "es") {
    return {
      subject: `Pago registrado: ${name}`,
      body: `¡Buen trabajo! Registramos tu pago de ${amt} hacia ${name}.\nPago aplicado a: ${name}.\nTu progreso fue actualizado en DebtYa.\n\nRecuerda: DebtYa no mueve dinero ni ejecuta pagos. Tú hiciste el pago fuera de DebtYa y nosotros actualizamos tu progreso.`
    };
  }
  return {
    subject: `Payment recorded: ${name}`,
    body: `Great job! We recorded your ${amt} payment toward ${name}.\nPayment applied to: ${name}.\nYour progress was updated in DebtYa.\n\nReminder: DebtYa does not move money or make payments. You made the payment outside DebtYa and we updated your progress.`
  };
}

function buildMinimumPaymentDueEmailPreview(opts = {}) {
  const lang = normalizePreferredLanguage(opts.preferredLanguage ?? opts.lang ?? "en");
  const es = lang === "es";
  const name = visibleDebtName(opts.debtName || opts.debt_name || "Debt");
  const amount = safeNumber(opts.amount, 0);
  const amountLine = amount > 0 ? formatMoneyTransactionalEmail(amount, lang) : null;
  if (es) {
    const lines = [
      "Hola,",
      "",
      `Hoy es el día del pago mínimo programado para ${name}.`,
      "DebtYa no hace el pago por ti.",
      "Si tienes activado el registro automático de pagos mínimos, DebtYa registrará el monto mínimo que configuraste y actualizará tu progreso."
    ];
    if (amountLine) lines.splice(4, 0, `Monto mínimo configurado: ${amountLine}`);
    return {
      channel: "email",
      subject: `DebtYa: hoy toca el pago mínimo de ${name}`,
      message: lines.join("\n"),
  ...
  const lines = [
    "Hello,",
    "",
    `Today is your scheduled minimum payment day for ${name}.`,
    "DebtYa does not make the payment for you.",
    "If automatic minimum payment tracking is enabled, DebtYa will record the minimum payment amount you set and update your progress."
  ];
  if (amountLine) lines.splice(4, 0, `Minimum payment amount you set: ${amountLine}`);
  return {
    channel: "email",
    subject: `DebtYa: minimum payment due today for ${name}`,
    message: lines.join("\n"),
    email_body: lines.join("\n"),
    debt_name: name,
    amount
...
async function sendMinimumPaymentDueEmail(deps = {}) {
  const {
    supabaseAdmin,
    userId,
    userEmail,
  ...
  const to = userEmail || (supabaseAdmin && userId ? await fetchAuthUserEmail(supabaseAdmin, userId) : null);
  if (!to || !String(to).includes("@")) {
    return { ok: true, sent: false, skipped: true, reason: "email_recipient_missing" };
  }
  const preview = buildMinimumPaymentDueEmailPreview({
    preferredLanguage,
    debtName,
    amount
  });
  ...
  try {
    const result = await sendMail({ to, preview, env, http });
    if (result && result.sent === true) {
      return { ok: true, sent: true, user_email: to, preview };
    }
    return {
      ok: true,
      sent: false,
      skipped: true,
      reason: result?.reason || "provider_returned_not_sent",
      user_email: to,
      preview
    };
  } catch (e) {
    if (typeof appError === "function") {
      try {
        appError("minimum payment due email failed:", e);
      } catch (_) {}
    }
    return {
      ok: false,
...
function buildDebtPaidOffTransactionalCopy(lang, debtNameRaw) {
  ...
  if (lang === "es") {
    return {
      subject: `¡Felicidades! Pagaste ${name} por completo`,
  ...
  return {
    subject: `Congrats! You paid off ${name}`,
...
async function sendTransactionalPaymentCelebrationEmails(deps) {
  ...
  try {
    ...
    out.ok = false;
    out.reason = "provider_send_failed";
