...
/**
 * Spinwheel expone bill pay en `liability.payments.billPayment.availability` (p. ej. SUPPORTED).
 * Mantener fallback a `capabilities.payments` por respuestas/cache antiguas.
 * @param {unknown} item
 */
/**
 * Valor en mayúsculas de `availability` para bill pay (p. ej. SUPPORTED, NOT_SUPPORTED) o cadena vacía.
 * @param {unknown} item
 */
function getBillPaymentAvailability(item) {
  if (!item || typeof item !== "object") return "";
  const root = /** @type {any} */ (item);
  const liab = root.liability && typeof root.liability === "object" ? root.liability : null;
  const payL = liab && liab.payments;
  const bpL = payL && payL.billPayment;
  const avL = bpL && bpL.availability != null ? String(bpL.availability).trim().toUpperCase() : "";

  const cap = root.capabilities;
  const payC = cap && cap.payments;
  const bpC = payC && payC.billPayment;
  const avC = bpC && bpC.availability != null ? String(bpC.availability).trim().toUpperCase() : "";

  return avL || avC || "";
}

function pickBillPaymentSupported(item) {
  return getBillPaymentAvailability(item) === "SUPPORTED";
}
