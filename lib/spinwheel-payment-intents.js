...
function maybePatchStatusConstraint(payload, error) {
  const e = /** @type {any} */ (error || {});
  if (String(e.code || "") !== "23514") return false;
  const msg = formatPostgrestError(e).toLowerCase();
  if (!msg.includes("payment_intents_status_check")) return false;
...
async function insertSpinwheelIntentResilient(supabaseAdmin, row, nowIso) {
  ...
  while (attempts < 12) {
    ...
    if (maybePatchStatusConstraint(work, r.error)) {
      continue;
    }
    return r;
  }
