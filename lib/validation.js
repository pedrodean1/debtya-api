/** Forma 8-4-4-4-12 (acepta v1–v8 y variantes sin validar version/variant por nibble). */
const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

const MAX_MONEY = 1e14;

const DEBT_SOURCES = new Set(["manual", "plaid", "method", "spinwheel"]);
...
function validateDebtCreatePayload(payload) {
  ...
  if (src === "spinwheel") {
    const xid = payload.spinwheel_external_id != null ? String(payload.spinwheel_external_id).trim() : "";
    if (!xid) {
      return "spinwheel_external_id requerido cuando source es spinwheel";
    }
  }
  return null;
}
